...
purchases.by.brand.per.household
...
```{r}
# PRICE
reduced.data$PRICE_PER_OUNCE<-reduced.data$UNIT_PRICE / reduced.data$VOL_EQ
...
test_panel_info <- panelists.with.income
test_panel_info$PANID <- as.character(test_panel_info$PANID)
test_data.subset <- data.subset
test_data.subset$PANID <- as.character(test_data.subset$PANID)

test_data.subset$VEND <- as.character(test_data.subset$VEND)
test_panel_info$PANID <- as.character(test_panel_info$PANID)
...
data.subset[data.subset$PANID == "1121723",]
panelists.with.income[669,]$PANID
paste("loyalty", "yu", sep="_")

sapply(1:nrow(panelists.with.income), function(x) {getLoyaltyByPanelistVendor(vendor = 33200, panelist = panelists.with.income[x,]$PANID)})
...
test <- merge(x=test_panel_info, y=test_data.subset[data.subset$VEND=="33200",], by=c("PANID"), all.x=TRUE)

merge(x=data.subset, y=panelists.with.income, by=c("PANID", "WEEK"), all.x = TRUE)
